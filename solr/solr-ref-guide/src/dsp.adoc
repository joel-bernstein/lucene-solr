...
In the next examples perform auto-correlation with each of the signals to see what the what plots look like.

In the example below the `conv` function is used to auto-correlate the sine wave. Notice that the `conv` function
...
In the example below auto-correlation is performed with the vector of pure noise.
...
In the example below auto-correlation is performed with the vector with sine wave hidden within the noise.
