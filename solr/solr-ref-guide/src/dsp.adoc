...
In the first example the `sin` function is wrapped around a `sequence` function to generate sine wave. The result of this
is plotted in the image below. Notice that there is a structure to plot that is clearly not random.
...
In the next example the `sample` function is used to draw 256 samples from a `uniformDistribution` to create a random
vector of data. The result of this is plotted in the image below. Notice that there is no clear structure to the
data and the data clearly appears to be random.
...
somewhat within the noise. Its difficult to say for sure if there is structure. As the plot
...
In the next examples autocorrelation is performed with each of the signals to see what the
what the autocorrelation plots look like.

In the example below the `conv` function is used to autocorrelate the sine wave. Notice that the `conv` function
is simply correlating the sine wave with itself.

The plot has a very distinct structure to it. As the sine wave is slid across a copy of it self the correlation
...
plot has a very different plot the sine wave. In this plot long period of low intensity correlation that appears
to be random. Then in the center a peak of high intensity where vectors are directly lined up. This is followed by
another long period of low intensity correlation.
...
In the example below autocorrelation is performed with the vector with sine wave hidden within the noise.
Notice that this plot shows a very clear signs of structure which is similar to autocorrelation plot of the
...
The functions described earlier in the section of the user guide all operate on signals in time domain. In the time
...
at different frequencies. The Discrete Fourier Transform decomposes a single time domain signal into its component
frequencies and measures the power at each frequency.

The discrete Fourier Transform has many important uses. In the example below discrete Fourier transform is used for
...
of the `fft` function is returned as *complex* numbers. A complex number has two parts a *real* and
an *imaginary* part. The imaginary part of the complex number is ignored in the examples below but there
are many tutorials about the FFT and complex numbers available online.

But it is important to understand how the `fft` function formats the complex numbers in the result.
...
Rather then introducing complex number abstraction into the language, the `fft` result is
...
the matrix which is a vector with real values of the fft response.

The plot of the real values of the `fft` response is shown below. Notice there are two
peaks on opposite sides of the plot. The plot is actually showing a mirrored response. The left side
of the plot is an exact mirror of the right side. This is expected when an fft is run on real rather then
complex data.
...
Notice that is this response there is no clear peak. Instead all frequencies have accumulated random level of
...
In the third example the `fft` function is called on the same signal hidden with noise that was used for
the autocorrelation example. The plot of the real values of the `fft` response is shown below.

Notice that there are two clear mirrored peaks, at the same locations as the `fft` on the pure signal. But
there is also now considerable noise on the frequencies. The `fft` has found the signal and but also
signals that there is considerable noise along with the signal.
