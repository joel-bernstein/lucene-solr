...
The `percentile` function returns an estimated value for a specific percentile in
a numeric array. The example below returns the estimation for the 95th percentile
...
The `cov` function calculates the covariance of two
numeric arrays.
...
if a matrix passed as a parameter. The correlation matrix is built by correlating the columns
...
* `mannWhitney`: The Mann-Whitney test is a non-parametric test that tests if two samples were pulled
