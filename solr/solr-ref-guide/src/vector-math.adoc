...
This section of the user guide deals with vector math and
...
Arrays can be passed as parameters to functions that operate on arrays. There are set of functions
that are designed to manipulate arrays.
...
copies an array from a start and end range.
...
The example below performs the l2norm calculation of an array.
...
The following vector math functions perform these operations: `scalarAdd`, `scalarSubtract`, `scalarMultiply`
and `scalarDivide`.


Below is an example the `scalarMultiply` function, which is multiplying the scalar value 3 with
...
vector math functions. The following element-by-element vector math functions perform
these operations: `ebeAdd`, `ebeSubtract`, `ebeMultiply`, `ebeDivide`.
