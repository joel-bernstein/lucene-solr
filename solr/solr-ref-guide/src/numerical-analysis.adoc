...
The `derivative` function returns the derivative of each control point of an
interpolated curve. The `derivative` function can also return a derivative of
a derivative.

The example below computes a derivative of a `loess` interpolation.
...
The `integrate` function computes an integral for specific
...
Notice that the data is not included in the matrix for year 2003 floor 8. The `bicupicSpline` function
