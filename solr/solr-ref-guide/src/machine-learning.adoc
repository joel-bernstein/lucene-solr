...
In the example, the centroid matrix is set to variable *d*. The first
centroid is selected from the matrix with the `rowAt` function.
Next the first cluster matrix is selected and set to variable *f*.
Then the `knn` function is used to find the 3 nearest neighbors
to the centroid vector in the cluster matrix.

The `knn` function returns a matrix with the 3 nearest neighbors.
Finally the top features of the nearest neighbor matrix
are returned.
