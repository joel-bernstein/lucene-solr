...
Time series analysis is one of the primary data analysis use cases. The `timeseries`
expression provides fast distributed time series aggregations, which can be
vectorized and analyzed with math expressions.

=== `knnSearch`

The K-nearest neighbor algorithm is one of the core machine learning algorithms. The `knnSearch`
...
Note that this section deals with the creation matrices from numeric data.
The next chapter of the user guide covers Text Analysis and Term Vectors
which describes how to build tf-idf term vector matrices from text fields.

Below is a simple example where four random samples are taken from different
sub-populations in the data. The *price_f* field of each random sample is
vectorized and the vectors are added as rows to a matrix. Then the `sumRows`
function is applied to the matrix to return a vector containing the sum
of each row.
